...
| **UI** | **Shop on Instacart** on trip-plan sections that are not a Kroger cart write or a Walmart add-to-cart link (Target, Aldi, regional/Flipp/partner banners, and Walmart search fallback). Plus “or shop the whole list on Instacart” on the trip total. Hidden when the env vars are unset. |
