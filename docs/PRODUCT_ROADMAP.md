...
- Pluggable store pricing providers: Kroger (live), Walmart Affiliate API (live when keys are set), Target licensed partner feed (live only with a contracted base URL), Flipp weekly-ad deals (Aldi / Target-without-partner-feed / Publix-like banners when enabled), env-gated **partner feed stubs** for Instacart-style / shelf-feed banners (see [Partner integrations](#partner-integrations-nationwide-coverage))
- Seed catalog samples for Aldi, Walmart, Kroger, and Target — labeled **Demo catalog** until a live, partner-feed, or weekly-ad provider returns rows
- Dashboard badges + `pricingByStore` so **Live prices**, **Partner feed**, **Weekly ad**, and **Demo catalog** stay distinct
...
- Dashboard badges: **Live prices** / **Partner feed** / **Cached live** / **Weekly ad** / **Demo catalog**. Weekly ad is flyer/circular pricing, not a full shelf feed.
...
| Piece | Behavior |
| --- | --- |
| Provider | `FlippDealsProvider` behind `StorePricingProvider`. Dedicated retailer APIs and licensed partner feeds still run first (Kroger Products, Walmart Affiliate, Target partner feed, `SHELF_FEED_*` / named banner stubs). |
...
## Partner integrations (nationwide coverage)

Official APIs first; licensed partners when a chain has no public API. **Do not scrape authenticated storefronts** as the production cart-fill path. **Do not report cart success** without a retailer/partner HTTP 2xx.

Playbook (who to call, what we need, price vs cart split, Costco/club honesty): **[`docs/PARTNER_INTEGRATIONS.md`](PARTNER_INTEGRATIONS.md)**.

| Source | Role | Env (dark until set) |
| --- | --- | --- |
| **Datasembly-class shelf feed** | Multi-banner **in-store** prices | `SHELF_FEED_BASE_URL`, `SHELF_FEED_API_KEY`, `SHELF_FEED_STORES` |
| **Instacart Connect/Platform** | Multi-banner **fulfillment**; prices often marked up | `INSTACART_PARTNER_BASE_URL`, `INSTACART_PARTNER_API_KEY`, `INSTACART_PARTNER_STORES` |
| **Publix / H-E-B / Meijer** | Single-banner licensed feeds (same HTTP contract as Target) | `PUBLIX_PARTNER_*`, `HEB_PARTNER_*`, `MEIJER_PARTNER_*` |
| Albertsons / Safeway, Ahold (Food Lion, Giant, Stop & Shop) | List banners on `SHELF_FEED_STORES` or Instacart stores | — |
| Costco / Sam’s, Amazon Fresh / Whole Foods | Search handoff only; **no** third-party cart-write | optional shelf/Instacart list, labeled honestly |
| **Flipp weekly ads** | Wired circular prices (**Weekly ad**, not a shelf file) | `FLIPP_ACCESS_TOKEN` / `FLIPP_ENABLED` — see Phase 2b |

Code: `PartnerFeedProvider` in `src/pricing/partnerFeed.ts` implements `GET {base}/products?query=&zip=&store=` with a Bearer key. That is Grocery Gitter’s adapter contract — **not** a fake Instacart/Datasembly SDK. When credentials arrive, point `*_BASE_URL` at a proxy that maps vendor JSON, or replace the client.

Configured partner banners join the default competitor list. Unconfigured banners are omitted from that extra list (Flipp-mapped Publix-like banners can still appear when you pass `stores`). Dashboard label for licensed stub hits: **Partner feed**. Licensed shelf feeds beat Flipp weekly ads for the same banner.

Handoff for those banners is public **search deep links** (or Aldi **Coming soon**). Cart fill stays Kroger shopper OAuth only.
...
| **Costco / Sam’s / Amazon Fresh / Whole Foods** | Effectively no third-party cart-write | Search + membership copy only. See the partner playbook. |
